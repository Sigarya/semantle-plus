...
serve(async (req)=>{
  ...
  try {
    ...
    if (!guess) {
      ...
      return new Response(JSON.stringify({
        error: "Missing required parameter: guess"
      }), {
        status: 400,
        headers: {
          "Content-Type": "application/json",
    ...
    // --- FEATURE IMPLEMENTATION: Sanitize the guess by removing all geresh characters ---
    const processedGuess = guess.replace(/'/g, "");
    console.log(`Original guess: "${guess}", Processed guess: "${processedGuess}"`);
    ...
    if (!target) {
      ...
      const { data: wordData, error: wordError } = await supabase.rpc('get_active_word_for_date', {
    ...
    try {
      // --- CHANGED: Use the processedGuess for the API call ---
      const similarityUrl = `${SIMILARITY_API_URL}/similarity?word1=${encodeURIComponent(processedGuess)}&word2=${encodeURIComponent(target)}`;
      ...
      // --- CHANGED: Use the processedGuess for the API call ---
      const rankingUrl = `${RANKING_API_URL}?word=${encodeURIComponent(processedGuess)}&date=${encodeURIComponent(formattedDate)}`;
      ...
      // --- CHANGED: Use the processedGuess to determine if the word is correct ---
      const isCorrect = processedGuess === target || similarity > 0.99;
      ...
      // --- CHANGED: Return the processedGuess to the user ---
      return new Response(JSON.stringify({
        word: processedGuess,
        similarity,
        rank,
